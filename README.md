...
👉 **https://islanddddddd.github.io/novel-copilot/**
...
本项目是**纯前端应用**，无需后端服务器，可直接部署为静态站点。

### 纯前端的优势

- **零服务器成本**：部署到 GitHub Pages 即可使用
- **数据完全私有**：所有数据存储在浏览器本地，不会上传到任何服务器
- **离线可用**：除 AI 请求外，所有功能均可离线使用

### 纯前端的限制

- **数据仅在当前浏览器**：无法跨设备同步，清除浏览器数据会丢失所有内容
- **存储容量有限**：localStorage 约 5-10MB，大量小说可能触及上限
- **无法实现多用户协作**：仅支持单机使用
- **API Key 暴露风险**：虽然保存在本地，但仍在前端代码中

> 💡 **提示**：如果你需要后端功能（如数据同步、多用户、更大存储），可以用 AI 辅助将本项目改造成全栈应用。项目结构清晰，React + TypeScript 代码易于扩展。
...
1. 点击右上角 **设置** → **AI 配置**
2. 添加你的 API Key（支持兼容 OpenAI 接口协议）
3. 点击 **测试连接**，获取可用模型列表
4. 选择模型，保存配置
